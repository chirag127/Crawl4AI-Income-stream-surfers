---
url: https://developer.chrome.com/blog/new-in-devtools-131?hl=en
title: https://developer.chrome.com/blog/new-in-devtools-131?hl=en
date: 2025-05-11T16:58:17.074340
depth: 2
---

[ Skip to main content ](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#main-content)


  * On this page
  * [Debug CSS with Gemini](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#ai-assistance)
    * [Control AI features in a dedicated settings tab](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#ai-innovations)
    * [Console insights are one click away](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#console-insights)
  * [Performance panel improvements](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#perf)
    * [Annotate and share performance findings](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#annotations)
    * [Get performance insights right in the Performance panel](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#insights)
    * [Spot excessive layout shifts easier](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#cls)
    * [Spot the non-composited animations](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#animations)
    * [Hardware concurrency moves to Sensors](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#hardware-concurrency)
  * [Ignore anonymous scripts and focus on your code in stack traces](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#ignore-list)
  * [Elements > Styles: Support for sideways-* writing modes for grid overlays and CSS-wide keywords](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#styles)
  * [Lighthouse audits for non-HTTP pages in timespan and snapshot modes](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#lighthouse)
  * [Miscellaneous highlights](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#misc)
  * [Download the preview channels](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#preview-channels)
  * [Get in touch with the Chrome DevTools team](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#contact-us)
  * [What's new in DevTools](https://developer.chrome.com/blog/new-in-devtools-131?hl=en#whats-new)
...
The new panel will prompt you to turn on the corresponding setting. Make sure you meet the [Requirements](https://developer.chrome.com/docs/devtools/ai-assistance#requirements), turn on the setting toggle, and go back to the **AI assistance** panel. It will take the element you selected as context. Type your question about the element.
To learn more about how best to use the new panel, see [5 Cool Things To Do with DevTools AI Assistance](https://developer.chrome.com/blog/5-cool-things-to-do-with-ai-assistance) and check out [AI assistance for styling](https://developer.chrome.com/docs/devtools/ai-assistance/styling).
The DevTools team is looking forward to hearing your feedback. Feel free to leave it in [crbug.com/364805393](https://crbug.com/364805393).
### Control AI features in a dedicated settings tab
You can now manage all AI features in one place: the new settings **Settings** > **AI innovations** tab. It lists important considerations, describes AI features, and lets you turn them on and off individually.
For more information see, [Settings > AI innovations](https://developer.chrome.com/docs/devtools/settings/ai-innovations).
...
You can now discover actionable insights in the new **Insights** tab on the left sidebar of the **Performance** panel. The insights are consolidated from the Lighthouse report and the **Performance insights** panel that is [about to be deprecated](https://developer.chrome.com/blog/perf-tooling-2024#the_experimental_performance_insights_panel_will_be_deprecated).
...
The DevTools team looks forward to your feedback on the usefulness of insights, ways to improve it, and your experience using them with other tooling, like [PageSpeed Insights](https://developers.google.com/speed) and [Lighthouse](https://developer.chrome.com/docs/lighthouse/overview). Feel free to leave your feedback in [crbug.com/371170842](https://crbug.com/371170842).
### Spot excessive layout shifts easier
The **Layout shifts** track gets a new look. The layout shifts are now marked with (more visible) purple diamonds and are [grouped in clusters based on their proximity on the timeline](https://web.dev/blog/evolving-cls). Both shifts and their clusters get an organized table with timings, scores, elements, and potential culprits in the **Summary** tab.
Additionally, the live metrics view gets the **Layout shifts** log with scores and elements next to the **Interactions** tab.
Chromium issue: [369100729](https://crbug.com/369100729).
...
For more information, see [Stick to Compositor-Only Properties and Manage Layer Count](https://web.dev/articles/stick-to-compositor-only-properties-and-manage-layer-count).
Chromium issue: [41006273](https://crbug.com/41006273).
...
Chromium issue: [371463665](https://crbug.com/371463665).
...
You can also turn on the new settings **Settings** > **Ignore list** > check_box **Anonymous scripts from eval or console** to set DevTools to ignore anonymous scripts without source URL.
Additionally, when you right-click and **Save as...** the console log, it doesn't save the **Show more/less** text.
Chromium issues: [40279542](https://crbug.com/40279542), [40945570](https://crbug.com/40945570), [345248263](https://crbug.com/345248263).
...
The **Elements** > **Styles** tab now supports the following:
  * Grid overlay in the viewport now displays grids for [`sideways-rl`](https://developer.mozilla.org/docs/Web/CSS/writing-mode#sideways-rl) and [`sideways-lr`](https://developer.mozilla.org/docs/Web/CSS/writing-mode#sideways-lr) writing modes. 
  * Resolves [CSS-wide keywords](https://developer.mozilla.org/docs/Web/CSS/CSS_Values_and_Units#css-wide_values). Practically, it means that, for example, if `inherit` is a color, the **Styles** tab shows a color picker next to it. 


Chromium issues: [40280717](https://crbug.com/40280717), [40706051](https://crbug.com/40706051), [40501131](https://crbug.com/40501131).
...
Chromium issue: [372411090](https://crbug.com/372411090).
## Miscellaneous highlights
These are some noteworthy fixes and improvements in this release:
  * Throttling settings are now correctly synced between the **Performance** and **Network** panels ([370332090](https://crbug.com/37033209)).
  * **Application** > **Background services** > **Speculative loads** > **Rules** now has a `{}` pretty-print button similar to **Sources** > **Editor** ([40279147](https://crbug.com/40279147)).
  * **Live expressions** : Pressing `Tab` after selecting an autocomplete option now exits the edit field instead of indenting the text ([349939551](https://crbug.com/349939551)).
  * **Elements** > **Styles** : [`anchor()`](https://www.w3.org/TR/css-anchor-position-1/#anchor-pos) and [`anchor-size()`](https://www.w3.org/TR/css-anchor-position-1/#anchor-size-fn) supports new syntax where you can re-order arguments and omit `anchor-size()` direction ([343516786](https://crbug.com/343516786)). Additionally, fixed fallback rendering ([365802559](https://crbug.com/365802559)).
  * **Network** : Fixed GraphQL previews ([369931288](https://crbug.com/369931288)).
  * **Performance** : Now reports incremental progress of loading and processing traces.
  * **WebAuthn** : Now dynamically updates credentials modified by `signal*` methods ([368467199](https://crbug.com/368467199)).
  * WebAssembly: A warning in the **Console** now informs you if multiple debug symbols are available for a WebAssembly module and which one is in use ([40879198](https://crbug.com/40879198), [369515221](https://crbug.com/369515221)).
  * The Core Web Vitals overlay is removed from the **Rendering** tab [328487897](https://crbug.com/328487897).

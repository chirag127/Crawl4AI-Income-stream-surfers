---
url: https://developer.chrome.com/blog/new-in-devtools-128?hl=en
title: https://developer.chrome.com/blog/new-in-devtools-128?hl=en
date: 2025-05-11T16:58:04.110792
depth: 2
---

[ Skip to main content ](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#main-content)
  * [Español – América Latina](https://developer.chrome.com/blog/new-in-devtools-128?hl=es-419)




  * On this page
  * [Console insights by Gemini are going live in most European countries](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#console-insights)
  * [Performance panel updates](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#perf)
    * [Enhanced Network track](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#perf-network)
    * [Customize performance data with extensibility API](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#perf-extension)
    * [Details in the Timings track](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#timings-details)
  * [Copy all listed requests in the Network panel](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#copy-all-listed)
  * [Faster heap snapshots with named HTML tags and less clutter](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#heap-snapshots)
  * [Open Animations panel to capture animations and edit @keyframes live](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#animations)
  * [Lighthouse 12.1.0](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#lighthouse)
  * [Miscellaneous highlights](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#misc)
  * [Download the preview channels](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#preview-channels)
  * [Get in touch with the Chrome DevTools team](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#contact-us)
  * [What's new in DevTools](https://developer.chrome.com/blog/new-in-devtools-128?hl=en#whats-new)
...
This version brings support of [console insights](https://developer.chrome.com/docs/devtools/console/understand-messages) by Gemini to most European countries.
...
This version brings support for extension data to the **Performance** panel. You can now add custom tracks with events and tooltip descriptions to the performance trace, details to the **Summary** tab, [and more](https://bit.ly/rpp-e11y). This feature may be useful for developers of frameworks, libraries, and complex applications with custom instrumentation.
See an example of a custom track on this [demo page](https://corgi-collage.vercel.app/). In **Performance** > **Capture settings** , turn on check_box **Extension data**. Start a performance recording, click **Add new Corgi** on the demo page, then stop the recording. You'll see a custom track in the trace that contains events with custom tooltips and details in the **Summary** tab.
In short, to extend performance data, pass a [specific structure](https://bit.ly/rpp-e11y#bookmark=id.6i6pg2kkkq4b) to the `measureOption.detail` or `markOption.detail` parameters of [`performance.measure()`](https://developer.mozilla.org/docs/Web/API/Performance/measure) or [`performance.mark()`](https://developer.mozilla.org/docs/Web/API/Performance/mark) API calls.
### Details in the Timings track
If you are a web developer who uses the [User Timing part of Performance API](https://developer.mozilla.org/docs/Web/API/Performance_API/User_timing) to add entries to the **Timings** track, you can now see your arbitrary details in the **Summary** tab for `mark` and `measure` events and their timestamps.
Chromium issue: [345418915](https://crbug.com/345418915).
...
Chromium issues: [41490040](https://crbug.com/41490040), [343341610](https://crbug.com/343341610), [42203857](https://crbug.com/42203857).
...
The **Animations** panel now does the following:
  ...
  * Supports live editing of [`@keyframes`](https://web.dev/learn/css/animations). In other words, updates the captured animation as you edit `@keyframes` section in **Elements** > **Styles**.


See both features in action in the following video.
Chromium issue: [352718055](https://crbug.com/352718055).
## Lighthouse 12.1.0
The **Lighthouse** panel now runs Lighthouse 12.1.0.
This update brings a number of changes, including the [removal of the old First Meaningful Paint (FMP)](https://github.com/GoogleChrome/lighthouse/pull/16046) metric in favor of [Largest Contentful Paint (LCP)](https://web.dev/articles/lcp). See the [full list of changes](https://github.com/GoogleChrome/lighthouse/releases/tag/v12.1.0).
...
Chromium issues: [349939551](https://crbug.com/349939551), [343942719](https://crbug.com/343942719), [349334243](https://crbug.com/349334243), [349428374](https://crbug.com/349428374).
## Miscellaneous highlights
These are some noteworthy fixes and improvements in this release:
  * **Performance** : 
    * **Network** track: Added WebSocket connection events [331351979](https://crbug.com/331351979).
    * The **Performance** panel now correctly zooms in on the busiest main thread activity [345599356](https://crbug.com/345599356).
    * Fixed a bug with uploading wrong trace file types, it now prevents uploading any type except the correct `.json` or `.gz` [349864878](https://crbug.com/349864878).
  * **Elements** > **Styles** : 
    * Unit drop-down in length property values is now deprecated [41495618](https://crbug.com/41495618).
    * Fixed resolving active properties for nested at-rules [346732737](https://crbug.com/346732737).
    * Inactive `@position-try` sections are now grayed out [40246493](https://crbug.com/40246493).
  * **Application** : 
    * **Cookies** : Fixed a bug that wouldn't refresh cookies upon **Refresh** click [348683488](https://crbug.com/348683488).
    * **Local storage** : You can now sort by keys alphabetically [341129585](https://crbug.com/341129585).

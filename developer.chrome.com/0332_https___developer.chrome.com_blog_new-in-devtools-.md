---
url: https://developer.chrome.com/blog/new-in-devtools-125?hl=en
title: https://developer.chrome.com/blog/new-in-devtools-125?hl=en
date: 2025-05-11T16:57:55.761361
depth: 2
---

[ Skip to main content ](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#main-content)
  * [Español – América Latina](https://developer.chrome.com/blog/new-in-devtools-125?hl=es-419)




  * On this page
  * [Understand errors and warnings in the Console better with Gemini](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#understand-messages)
  * [@position-try rules support in Elements > Styles](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#position-try)
  * [Sources panel improvements](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#sources)
    * [Configure automatic pretty-printing and bracket closing](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#settings)
    * [Handled rejected promises are recognized as caught](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#caught-rejection)
  * [Error causes in the Console](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#error-cause)
  * [Network panel improvements](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#network)
    * [Inspect Early Hints headers](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#early-hints)
    * [Hide the Waterfall column](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#waterfall)
  * [Performance panel improvements](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#perf)
    * [Capture CSS selector statistics](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#selector-stats)
    * [Change order and hide tracks](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#track-config)
  * [Ignore retainers in the Memory panel](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#ignore-retainer)
  * [Lighthouse 11.7.1](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#lighthouse)
  * [Miscellaneous highlights](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#misc)
  * [Download the preview channels](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#preview-channels)
  * [Get in touch with the Chrome DevTools team](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#contact-us)
  * [What's new in DevTools](https://developer.chrome.com/blog/new-in-devtools-125?hl=en#whats-new)
...
To get an AI-generated explanation of an error or warning, click the **Understand this error (warning)** button next to the message in the **Console** and follow instructions.
For more information, see [Understand errors and warnings better with AI](https://developer.chrome.com/docs/devtools/console/understand-messages).
## `@position-try` rules support in Elements > Styles
To help you debug [CSS anchor positioning](https://developer.chrome.com/blog/chrome-125-beta#css_anchor_positioning), the **Elements** > **Styles** tab now supports [`@position-try` CSS rules](https://developer.chrome.com/blog/anchor-positioning-api#adjust_anchor_positions_with_position-try). The tab resolves `position-try-options` values and links each option to a dedicated `@position-try --name` section.
To learn more, see [Introducing the CSS anchor positioning API](https://developer.chrome.com/blog/anchor-positioning-api).
Chromium issue: [40279608](https://crbug.com/40279608).
...
You can now turn on or off the automatic pretty-printing and bracket closing for the **Editor** in **Sources**. [Pretty-printing](https://developer.chrome.com/docs/devtools/javascript/reference#format) makes minified files readable. Bracket closing automatically adds a closing bracket (`)` or `}`) or tag (`>`) when you type an opening one.
To configure the relevant behaviour, check or clear the new check_box **Auto closing brackets** and check_box **Automatically pretty print minified sources** options in settings **Settings** > **Preferences** > **Sources**.
Chromium issues: [40865010](https://crbug.com/40865010), [324314570](https://crbug.com/324314570).
...
In other words, when **Sources** > **Breakpoints** > check_box **Pause on uncaught exceptions** is turned on, the debugger won't pause on statements similar to the following:
```
Promise.reject(newError('fail')).catch((e)=>console.log('caught'));

```

Chromium issue: [40283993](https://crbug.com/40283993).
## Error causes in the Console
The **Console** now shows you chains of [error causes](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error/cause) in the stack trace, if any.
To make debugging easier, you can [specify error causes](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Error/cause#examples) when catching and rethrowing errors. As the **Console** walks up the cause chain, it prints each error stack with a `Caused by:` prefix, so you can still see the original error.
Chromium issue: [40182832](https://crbug.com/40182832).
...
[Early Hints](https://developer.mozilla.org/docs/Web/HTTP/Status/103) is an HTTP status code (`103 Early Hints`) used to send a preliminary HTTP response ahead of a final response. This allows a server to send hints to the browser about critical sub-resources (for example, a style sheet or critical JavaScript) or origins that will be likely used by the page, while the server is busy generating the main resource.
For more information, see [Faster page loads using server think-time with Early Hints](https://developer.chrome.com/docs/web-platform/early-hints).
Chromium issue: [40222701](https://crbug.com/40222701).
### Hide the Waterfall column
You can now hide the **Waterfall** column in the **Network** panel similar to how you can hide other columns. Right-click any column name and clear the check_box_outline_blank **Waterfall** checkbox in the drop-down menu.
Chromium issue: [40574989](https://crbug.com/40574989).
...
Chromium issue: [324282954](https://crbug.com/324282954).
...
To enter the configuration mode, click the edit **Edit** button on the left of the track name. Then click arrow_upward up or arrow_downward down to move the track around or click visibility_off hide. Click the done **Check** button to the right of the track name when you are done.
The next version, Chrome 126, will bring more improvements to this UI.
Chromium issue: [311439339](https://crbug.com/311439339).
## Ignore retainers in the Memory panel
You can now ignore retainers in heap snapshots you capture with the **Memory** panel.
To ignore a retainer, select an object and, in the **Retainer** section, right-click a retainer and select **Ignore this retainer** from the drop-down menu. Ignored retainers are marked with`ignored` value in the **Distance** column. To stop ignoring, click **Restore ignored retainers** in the action bar at the top.
Additionally, heap snapshots now support a bigger number (hundreds of millions) of containment edges and nodes ([332350576](https://crbug.com/332350576)).
Chromium issue: [327337527](https://crbug.com/327337527).
## Lighthouse 11.7.1
The **Lighthouse** panel now runs Lighthouse 11.7.1. See the [full list of changes](https://github.com/GoogleChrome/lighthouse/releases/tag/v11.7.1).
...
These are some noteworthy fixes and improvements in this release:
  * The **Recorder** panel is now officially out of preview status ([329271496](https://crbug.com/329271496)).
  * The **Console** now doesn't show an error when a custom formatter returns a `null` for the `body()` function, which is a valid behavior ([329400119](https://crbug.com/329400119)).
  * The **Sources** panel updated syntax highlighter, in particular, it now supports `v` and `d` flags in regular expression.
  * The **Network** > **Cookies** tab fixed a bug with mapping exempted cookies to response cookies ([41491846](https://crbug.com/41491846)).
  * The **Elements** > **Styles** tab now does the following: 
    * Shows fully-overloaded inherited rules with custom properties ([41489874](https://crbug.com/41489874)).
    * Highlights the applied value in light-dark() depending on the color theme ([331123816](https://crbug.com/331123816)).

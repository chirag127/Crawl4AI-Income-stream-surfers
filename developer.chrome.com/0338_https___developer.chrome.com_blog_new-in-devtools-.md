---
url: https://developer.chrome.com/blog/new-in-devtools-129?hl=en
title: https://developer.chrome.com/blog/new-in-devtools-129?hl=en
date: 2025-05-11T16:58:16.909986
depth: 2
---

[ Skip to main content ](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#main-content)
  * [Español – América Latina](https://developer.chrome.com/blog/new-in-devtools-129?hl=es-419)




  * On this page
  * [Recorder supports export to Puppeteer for Firefox](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#puppeteer-export)
  * [Performance panel improvements](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#perf)
    * [Live metrics observations](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#observations)
    * [Search requests in the Network track](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#perf-network-search)
    * [See stack traces of performance.mark and performance.measure calls](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#extension-stack-traces)
  * [Use test address data in the Autofill panel](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#autofill-addresses)
  * [Elements panel improvements](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#elements)
    * [Force more states for specific elements](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#specific-element-states)
    * [Elements > Styles now autocompletes more grid properties](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#grid-autocomplete)
  * [Lighthouse 12.2.0](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#lighthouse)
  * [Miscellaneous highlights](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#misc)
  * [Download the preview channels](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#preview-channels)
  * [Get in touch with the Chrome DevTools team](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#contact-us)
  * [What's new in DevTools](https://developer.chrome.com/blog/new-in-devtools-129?hl=en#whats-new)
...
As part of [WebDriver BiDi](https://w3c.github.io/webdriver-bidi/) support, the **Recorder** panel can now export recordings to Puppeteer for Firefox. With [Puppeteer's support of Firefox](https://hacks.mozilla.org/2024/08/puppeteer-support-for-firefox/), you can now record user flows using the Chrome DevTools **Recorder** panel, export them, and run them against both Firefox and Chrome.
For more information, see [WebDriver BiDi - The future of cross-browser automation](https://developer.chrome.com/blog/webdriver-bidi).
## Performance panel improvements
This version brings a number of improvements to the **Performance** panel.
### Live metrics observations
The **Performance** panel now shows you live observations about [Core Web Vitals](https://web.dev/articles/vitals#core-web-vitals), both on your local machine and based on field data from [Chrome UX Report](https://developer.chrome.com/docs/crux). This lets you spot performance issues without having to capture performance traces and understand how representative your experience is compared to those of your users.
To see live observations on LCP and CLS, open the **Performance** panel. To see INP, perform an interaction on a page. To compare your local metrics to the aggregated user experience from Chrome UX Report, add field data: in the **Field data** section on the right, click **Set up** , and, in the dialog window, click **Ok**. Hover over a metric value to see a tooltip with more information.
...
The [**Search**](https://developer.chrome.com/docs/devtools/performance/reference#search) box in the **Performance** panel now also works across the **Network** track, so you can find requests with the `Ctrl` / `Cmd` + `F` shortcut.
...
For more information, see [Customize your performance data with extensibility API](https://developer.chrome.com/docs/devtools/performance/extension).
...
To autofill address forms with test data, open the **Autofill** panel, turn on check_box **Show test addresses in autofill menu** , right-click an address form filed on your page, and select one of the options from the **Developer tools** menu.
...
The `:hov` section in **Elements** > **Styles** now provides you with more pseudo-classes you can forcibly enable. The new set of options is under the **Force specific element** state drop-down and is specific to certain elements you select. For example, `<label>` and `<input>` have different sets of options.
Chromium issue: [40280012](https://crbug.com/40280012).
...
For more information, see [Inspect CSS grid layouts ](https://developer.chrome.com/docs/devtools/css/grid) and its [Show line names](https://developer.chrome.com/docs/devtools/css/grid#line-names) section in particular.
## Lighthouse 12.2.0
The **Lighthouse** panel now runs Lighthouse 12.2.0.
This update brings a number of bug fixes. See the [full list of changes](https://github.com/GoogleChrome/lighthouse/releases/tag/v12.2.0).
...
These are some noteworthy fixes and improvements in this release:
  * **Elements** : 
    * Fixed a bug with incorrect rendering of overloaded length properties [357020613](https://crbug.com/357020613).
    * `position-try-options` renamed into `position-try-fallbacks` as per [specification](https://github.com/w3c/csswg-drafts/issues/10395#issuecomment-2192127524).
    * A page refresh now restores the selected node even inside an iframe [40719145](https://crbug.com/40719145).
    * **Accessibility** : Screen readers will now announce the **Show element** button [357382536](https://crbug.com/357382536).
  * **Performance** > **Network** : The **Reveal in Network** menu option now opens the **Headers** tab of the relevan network request.
  * **Console** : 
    * Errors from C/C++ extension now don't forcibly open the **Console** [356320158](https://crbug.com/356320158).
    * Fixed a bug with `import.meta` in a JS module in not evaluating when paused [40743793](https://crbug.com/40743793).
  * **Memory** : Fixed a bug with Restore ignored retainers not showing up after ignoring a retainer [327337527](https://crbug.com/327337527).
